...
public class DBRead {
    //URL for our Sql server Database
    ...
    public void user_gets_the_table(String string, String string2) throws SQLException {
        //Running the query to connect tHOTEL table(HARD CODE)
    ...
    public void user_reads_all_rows_in_the_column(String string) throws SQLException {
        resultSet.next();//SKIP THE FİRST ROW.NEXT () GOES TO THE NEXT ROW
        resultSet.next();
        resultSet.next();
        Object object1 = resultSet.getObject(string);//Kodu dynamic yaptık
...
//        System.out.println(string2);
        resultSet.absolute(int1);//Obsolete is go to a certain row
        Object obj = resultSet.getObject(string);//Going to thr column
